-- ---------------------------------------------------------------------
-- Fase 27: grupos de observação por produto — diferente de complemento
-- (que soma preço), aqui é só uma escolha sem custo, tipo "ESCOLHA SEU
-- MOLHO" com opções Ketchup/Mostarda/Maionese marcadas por checkbox.
-- Cada produto pode ter vários grupos, cada grupo tem um título e uma
-- lista de opções em texto. A seleção do garçom vira uma lista de
-- textos gravada no item do pedido/venda, só pra aparecer na conta e
-- no ticket da cozinha (não afeta valor).
-- ---------------------------------------------------------------------

alter table produtos add column if not exists grupos_observacao jsonb not null default '[]';
alter table pedido_itens add column if not exists observacoes jsonb not null default '[]';
alter table venda_itens add column if not exists observacoes jsonb not null default '[]';

-- lancar_pedido_itens redeclarada: cada item pode trazer p_item->'observacoes'
-- (array de strings) além de complementos, gravado igual em pedido_itens.
...
begin
  ...
  loop
    ...
    loop
      insert into pedido_itens (rodada_id, produto_id, nome_produto, quantidade, preco_unitario, complementos, observacoes)
      values (
        v_rodada_id,
        ...
        coalesce(v_item->'observacoes', '[]'::jsonb)
      );
    end loop;
...
-- finalizar_pedido_mesa redeclarada: carrega pi.observacoes pra
-- venda_itens junto, pra a observação sobreviver no histórico.
...
begin
  ...
  for v_item in
    select pi.produto_id, pi.nome_produto, pi.quantidade, pi.preco_unitario, pi.complementos, pi.observacoes
    from pedido_itens pi join pedido_rodadas pr on pr.id = pi.rodada_id
    where pr.pedido_id = p_pedido_id and not pi.cancelado
  loop
    insert into venda_itens (venda_id, produto_id, nome_produto, quantidade, preco_unitario, complementos, observacoes)
    values (v_venda_id, v_item.produto_id, v_item.nome_produto, v_item.quantidade, v_item.preco_unitario, coalesce(v_item.complementos, '[]'::jsonb), coalesce(v_item.observacoes, '[]'::jsonb));
  end loop;
